...
class NewRegistrarSpec extends AnyFlatSpec with should.Matchers {
  ...
  "PlasmaMap" should "tester" in {
    ...
    println(bytesToHex(hashedTld))
    val updatedRegistrarsMapResult = registrarsMap.insert((hashedTld, "01"))

    println(bytesToHex(updatedRegistrarsMapResult.proof.bytes))

    val removeResult = registrarsMap.delete(hashedTld)

    println(bytesToHex(removeResult.proof.bytes))
  }
